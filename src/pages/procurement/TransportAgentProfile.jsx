...
const TransportAgentProfile = () => {
  ...
  return (
    <PageShell>
        ...
        <MainPanel className="flex-1 min-w-0 !pt-0">
          ...
          <section id="tap-overview" className="rounded-zarewa border border-gray-100 bg-white shadow-sm p-5 mb-8 scroll-mt-28">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
              <div className="rounded-lg border border-slate-200/90 bg-slate-50/40 px-3 py-3">
                <p className="text-ui-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Assignments</p>
                <p className="text-lg font-black text-zarewa-teal tabular-nums">{intel?.assignmentCount ?? 0}</p>
                <p className="text-ui-xs text-slate-500 mt-1">POs linked to this agent</p>
              </div>
              <div className="rounded-lg border border-slate-200/90 bg-slate-50/40 px-3 py-3">
                <p className="text-ui-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">With haulage fee</p>
                <p className="text-lg font-black text-zarewa-teal tabular-nums">{intel?.withTransportFeeCount ?? 0}</p>
              </div>
              <div className="rounded-lg border border-slate-200/90 bg-amber-50/50 px-3 py-3">
                <p className="text-ui-xs font-semibold text-amber-800 uppercase tracking-wider mb-1">On road / loading</p>
              ...
              <div className="rounded-lg border border-slate-200/90 bg-zarewa-teal text-white px-3 py-3">
                <p className="text-ui-xs font-semibold text-white/70 uppercase tracking-wider mb-1">Last PO date</p>
                <p className="text-lg font-black">{intel?.lastOrderISO || '—'}</p>
              </div>
            </div>
          </section>

          <section id="tap-profile" className="rounded-zarewa border border-zarewa-teal/15 bg-zarewa-teal/[0.03] p-5 mb-8 scroll-mt-28">
            <p className="text-ui-xs font-black text-zarewa-teal uppercase tracking-widest mb-3 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              <div className="flex items-start gap-2 rounded-xl border border-slate-200/70 bg-white/50 px-3 py-2.5">
                ...
                <div>
                  <span className="text-ui-xs font-bold text-gray-400 uppercase block">Phone</span>
              ...
              <div className="flex items-start gap-2 rounded-xl border border-slate-200/70 bg-white/50 px-3 py-2.5">
                ...
                <div>
                  <span className="text-ui-xs font-bold text-gray-400 uppercase block">Region / base</span>
              ...
              {p.emergencyContact ? (
                <div className="flex items-start gap-2 rounded-xl border border-slate-200/70 bg-white/50 px-3 py-2.5 sm:col-span-2">
                  ...
                  <div>
                    <span className="text-ui-xs font-bold text-gray-400 uppercase block">Emergency contact</span>
            ...
            <div className="mt-4 rounded-xl border border-teal-200/80 bg-teal-50/40 p-4">
              <p className="text-ui-xs font-black text-teal-900 uppercase tracking-wider mb-3 flex items-center gap-1">
                <Truck size={12} /> Fleet &amp; operations
              </p>
              <dl className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                <div>
                  <dt className="text-ui-xs font-bold text-slate-400 uppercase">Vehicle type</dt>
                  <dd className="font-semibold text-slate-800 mt-0.5">{p.vehicleType || '—'}</dd>
                </div>
                <div>
                  <dt className="text-ui-xs font-bold text-slate-400 uppercase">Registration</dt>
                  <dd className="font-mono font-semibold text-slate-800 mt-0.5">{p.vehicleReg || '—'}</dd>
                </div>
                <div>
                  <dt className="text-ui-xs font-bold text-slate-400 uppercase">Payment preference</dt>
                  <dd className="font-semibold text-slate-800 mt-0.5">{p.paymentPreference || '—'}</dd>
                </div>
              </dl>
              {p.typicalRoutes ? (
                <p className="text-xs text-teal-900/90 mt-3 leading-relaxed border-t border-teal-100/80 pt-3">
                  <span className="font-bold">Typical routes: </span>
                  {p.typicalRoutes}
                </p>
              ) : null}
              {p.reliabilityNotes ? (
                <p className="text-xs text-slate-700 mt-2 leading-relaxed border-t border-teal-100/80 pt-3">
          ...
          {intel ? (
            <section id="tap-metrics" className="rounded-zarewa border border-gray-100 bg-white shadow-sm p-5 mb-8 scroll-mt-28">
              ...
              <div className="mb-3 grid grid-cols-1 gap-2 text-ui-xs sm:grid-cols-4">
                ...
                <div className="rounded-lg border border-slate-100 bg-slate-50/80 px-2 py-1.5">
                  ...
                  <p className="text-xs font-bold text-zarewa-teal tabular-nums">{formatNgn(intel.totalTransportNgn)}</p>
                </div>
                <div className="rounded-lg border border-slate-100 bg-slate-50/80 px-2 py-1.5">
                  <p className="text-slate-500 font-semibold uppercase tracking-wide">Paid (file)</p>
                  <p className="text-xs font-bold text-slate-800 tabular-nums">{formatNgn(intel.totalPaidTransportNgn)}</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                <span className="rounded-md bg-sky-50 px-2 py-1 text-ui-xs font-bold text-sky-900">
                  Avg ₦/kg (weighted, coil):{' '}
                  {intel.weightedAvgTransportPerKgNgn != null
                    ? formatNgn(Math.round(intel.weightedAvgTransportPerKgNgn))
                    : '—'}
                </span>
                <span className="rounded-md bg-slate-100 px-2 py-1 text-ui-xs font-semibold text-slate-700">
              ...
              <p className="text-ui-xs text-slate-500 leading-snug mt-2">
                ...
                {intel.history.map((h) => (
                  <li key={h.poID}>
                    <button
                      ...
                      className="w-full text-left rounded-lg border border-slate-200 bg-slate-50/50 px-4 py-3 hover:border-zarewa-teal/40 hover:bg-zarewa-teal/[0.03] transition-colors"
                    >
                      ...
                      <div className="mt-1.5 flex flex-wrap gap-1.5 text-ui-xs">
      ...
      <ModalFrame isOpen={Boolean(selectedPo)} onClose={() => setSelectedPo(null)} showCloseButton={false}>
