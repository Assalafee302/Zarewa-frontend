...
const Reports = () => {
  ...
  const { domainLoading, domainReady } = useWorkspaceDomain(['finance', 'operations']);
  ...
  return (
    <PageShell>
      ...
      <PageHeader
        eyebrow="Print desk"
        title={pageTitle}
        subtitle="Pick a job, set the period, then print or download. Period packs live here; live GL stays on Accounting; workforce reports on HR."
      />
          ...
          {countOnlyOverview ? (
            <div className={`${PANEL}`}>
              ...
              <p className="mb-4 text-sm font-medium text-slate-600">
                Entity counts for your branch. Detailed packs need Sales, Procurement, Operations, or Finance access.
              </p>
              {summaryErr ? <p className="mb-3 text-sm font-semibold text-red-600">{summaryErr}</p> : null}
                ...
                <div className="grid grid-cols-2 gap-px overflow-hidden rounded-md border border-slate-200 bg-slate-200 sm:grid-cols-4">
                  {[
                    ...
                    <div key={label} className="bg-white px-3 py-2.5">
                      ...
                      <p className="z-stencil mt-0.5 text-lg text-slate-900">{Number(n) || 0}</p>
                    </div>
                  ))}
                </div>
              ) : null}
