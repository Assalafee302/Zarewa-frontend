...
const TABS = [
  { id: 'daily-roll', label: 'Daily Roll' },
...
export default function HrAttendanceHub() {
  const { tab, setTab } = useHrUrlTab('daily-roll', TABS.map((t) => t.id));

  return (
    <HrTabbedPage
      title="Attendance & Time Book"
      description="Daily roll call, attendance uploads, exceptions, and payroll deduction preview."
      tabs={TABS}
      tab={tab}
      onTabChange={setTab}
    >
      {tab === 'daily-roll' || tab === 'deduction-preview' ? (
        <HrAttendance
          embedded
          activeTab={tab === 'deduction-preview' ? 'deductions' : 'roll'}
          hideInternalTabs
        />
      ) : null}
      {tab === 'uploads' ? (
