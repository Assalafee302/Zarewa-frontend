...
import { MainPanel, PageHeader, PageShell, PageTabs, Breadcrumbs, ResponsiveTableShell } from '../../components/layout';
...
function EstChip() {
  return (
    <span
      className="inline-flex rounded px-1.5 py-0.5 text-ui-xs font-semibold text-amber-900 ring-1 ring-amber-200/80 bg-amber-50"
      title="Estimated from produced quotations, not posted GL"
    >
      Estimated
    </span>
  );
...
function MetricPair({ label, value }) {
  return (
    <div>
      <dt className="text-ui-xs text-slate-500">{label}</dt>
      <dd className="mt-0.5 text-sm tabular-nums text-slate-900">{value}</dd>
    </div>
  );
}

function KpiCard({ label, value, sub, icon, loading, estimated, warn }) {
  return (
    <div className="bg-white p-4 min-h-[108px]" aria-busy={loading || undefined}>
      <p className="text-ui-xs font-medium text-slate-500 flex items-center gap-1.5 flex-wrap">
        {icon ? <span className="text-slate-400" aria-hidden>{icon}</span> : null}
        {label}
        {estimated ? <EstChip /> : null}
      </p>
      {loading ? (
        <div className="mt-3 h-8 w-24 animate-pulse rounded bg-slate-200" aria-hidden />
      ) : (
        <p className={`z-stencil mt-2 text-2xl tabular-nums ${warn ? 'text-amber-900' : 'text-slate-900'}`}>
          {value}
        </p>
      )}
...
export default function ExecutiveCommandCentre() {
  ...
  const isExecutiveOversight = roleKey === 'md' || roleKey === 'ceo' || roleKey === 'chairman' || roleKey === 'admin';
  ...
  const branchOptions = useMemo(() => {
    ...
    for (const b of Array.isArray(list) ? list : []) {
      ...
      mapped.push({
        id,
        label: String(b.name || b.label || b.branchName || id).trim() || id,
      });
    }
    if (branchId && branchId !== 'ALL' && !seen.has(branchId)) {
      mapped.push({ id: branchId, label: branchId });
    }
    return [{ id: 'ALL', label: 'All branches' }, ...mapped];
  }, [ws?.snapshot?.workspaceBranches, ws?.session?.branches, branchId]);
  const branchLabelById = useCallback(
    (id) => branchOptions.find((b) => b.id === id)?.label || id || '',
    [branchOptions]
  );
    ...
    rawTab === 'customers'
      ...
      : rawTab === 'trace'
        ...
        : rawTab === 'intelligence'
          ...
          : rawTab === 'finance'
            ? 'finance'
            : rawReviewView === 'intelligence' ||
                rawReviewView === 'finance' ||
                rawReviewView === 'customers' ||
                rawReviewView === 'trace' ||
                rawReviewView === 'pack'
              ? rawReviewView
              : 'pack';
  const reviewView =
    requestedReviewView === 'intelligence' && !mayViewBi ? 'pack' : requestedReviewView;
  const activeTab =
    rawTab === 'today' && isMdCockpit
      ? 'today'
      : rawTab === 'decide' && isMdCockpit
        ? 'decide'
        : (rawTab === 'customers' || rawTab === 'trace') && isMdCockpit
          ? 'overview'
  ...
  const setActiveTab = (tabId) => {
    if (isMdOffice && (tabId === 'intelligence' || tabId === 'finance' || tabId === 'customers' || tabId === 'trace')) {
      setReviewView(tabId);
      return;
    }
  ...
  useEffect(() => {
    if (activeTab === 'overview' && reviewView === 'customers' && isMdCockpit) void loadCustomerIntel();
  }, [activeTab, isMdCockpit, loadCustomerIntel, reviewView]);

  useEffect(() => {
    if (activeTab === 'overview' && reviewView === 'trace' && isMdCockpit) void loadTrace(false);
  }, [activeTab, isMdCockpit, loadTrace, reviewView]);
  ...
  const branchTrendLabel = branchLabelById;
  ...
  const visibleTabs = (isIntelligenceOnly
    ...
    if (isMdCockpit && (t.id === 'customers' || t.id === 'trace')) return false;
    if (t.mdOnly && !isMdCockpit) return false;
    if (t.mdOnly && (roleKey === 'ceo' || roleKey === 'chairman')) return false;
  ...
  const pageTabs = visibleTabs.map((t) => ({
    id: t.id,
    label: t.label,
    badge: t.id === 'decide' ? workTrayMdOnlyCount : undefined,
  ...
  return (
    <MainPanel>
        ...
        <PageHeader
          ...
          toolbar={
            ...
            <>
              ...
              <span className="inline-flex rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-ui-xs font-semibold uppercase text-slate-600">
                {roleLabel}
              </span>
              {readOnly ? (
              ...
              <button type="button" onClick={() => navigate('/reports')} className={`${EXEC_SECONDARY_BTN} hidden sm:inline-flex`}>
      ...
      {err ? (
        <p className="mb-6 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800" role="alert">
          {err}{' '}
      ...
      {activeTab === 'overview' && !isMdOffice ? (
        <>
      <section className="mb-8 space-y-3" aria-label="Period snapshot">
        <div className="rounded-md border border-slate-200 bg-white p-4 sm:p-5">
          ...
          {busy && !data ? (
            <div className="mt-2 h-10 w-40 animate-pulse rounded bg-slate-200" aria-hidden />
          ) : (
            <p className="z-stencil mt-1 text-3xl text-slate-900 sm:text-4xl">{formatNgn(kpis.salesNgn ?? 0)}</p>
          )}
          <p className="mt-1 text-ui-xs text-slate-500">
            Estimated from produced quotations, not posted GL
          </p>
        </div>
        <div className="grid grid-cols-2 gap-px overflow-hidden rounded-md border border-slate-200 bg-slate-200 lg:grid-cols-4">
          ...
          <KpiCard
            ...
            sub="Waiting in the work tray"
            icon={<Shield size={12} />}
            loading={busy && !data}
            warn={Number(kpis.pendingExecutiveActions) > 0}
          />
          <KpiCard
            label="Critical alerts"
            value={String(kpis.criticalAlerts ?? 0)}
            sub="Needs a management decision"
            icon={<AlertTriangle size={12} />}
            loading={busy && !data}
      ...
      <div className="space-y-8 pb-12">
        <Section
          ...
          icon={<AlertTriangle size={18} className="text-amber-700" aria-hidden />}
        >
        ...
        <Section
          ...
          subtitle={
            showBranchComparison
              ? 'Comparison across locations for the selected period.'
          ...
          {branchComparisonEmpty ? (
            <p className="text-sm text-slate-600 rounded-xl border border-slate-200 bg-slate-50 px-4 py-6 text-center leading-relaxed">
              {data?.branches?.comparisonEmptyReason === 'single_branch'
                ? 'Select All branches to compare locations.'
              ...
              {(data?.branches?.byBranch || []).map((b) => (
                <li key={b.branchId}>
                  <article className="rounded-md border border-slate-200 bg-white p-3">
                    <h3 className="text-sm font-semibold text-slate-900">{b.branchName || b.branchId}</h3>
                    <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2">
                      <MetricPair label="Sales" value={formatNgn(b.producedRevenueNgn)} />
                      <MetricPair label="Collections" value={formatNgn(b.netCollectedNgn)} />
                      <MetricPair
                        label="Collection rate"
                        value={b.producedCollectionRatePct != null ? `${b.producedCollectionRatePct}%` : '—'}
                      />
                      <MetricPair label="Expenses" value={formatNgn(b.expensesNgn)} />
                      <MetricPair label="Receivables" value={formatNgn(b.customerDebtNgn)} />
                      <MetricPair label="Stock" value={formatNgn(b.coilValuationNgn)} />
                      <MetricPair label="Pending jobs" value={String(b.pendingProductionJobs ?? 0)} />
                      <MetricPair label="Risk / index" value={`${b.riskFlagCount ?? 0} / ${b.internalScore ?? '—'}`} />
                    </dl>
                  </article>
                </li>
              ))}
            ...
            <div className="hidden md:block">
              <ResponsiveTableShell scrollHint={false}>
        ...
        >
          ...
          {filteredWorkTrayItems.length === 0 && !busy ? (
            <p className="py-6 text-center text-sm text-slate-500 md:hidden">
              {workTrayFilter === 'all' ? 'No pending executive items.' : 'No items in this queue.'}
            </p>
          ) : (
            <ul className="space-y-3 md:hidden">
                ...
                <li key={row.id}>
                  <article className="rounded-md border border-slate-200 bg-white p-3">
                    <div className="flex flex-wrap items-center gap-1.5">
                      <span
                        className={`inline-flex rounded-md px-2 py-0.5 text-ui-xs font-semibold ring-1 ${priorityChip(row.priority)}`}
                      >
                        {row.priority}
                      </span>
                      <span
                        className={`inline-flex rounded-md px-2 py-0.5 text-ui-xs font-semibold ring-1 ${approvalTierChip(row.approvalTier)}`}
                      >
                    ...
                    >
                      {row.summaryOnly ? 'View summary' : row.canAct ? 'Review' : 'View'}
                    </button>
                  </article>
                </li>
              ))}
