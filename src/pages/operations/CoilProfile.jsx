...
import { CoilTicketHeader } from '../../components/operations/CoilTicketHeader.jsx';
  ...
  return (
    <PageShell>
      ...
      <CoilTicketHeader
        coilNo={coil.coilNo}
        colour={coil.colour}
        gauge={coil.gaugeLabel || coil.gauge}
        remainingM={
          (purchaseConversion || avgActualConversion || avgStandardConversion) > 0.01
            ? currentKg / (purchaseConversion || avgActualConversion || avgStandardConversion)
            : null
        }
        onHandKg={currentKg}
        freeKg={freeKg}
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start min-w-0">
        <aside className="w-full lg:w-56 shrink-0 lg:sticky lg:top-24 space-y-1">
          <p className="text-[11px] font-medium text-slate-500 px-3 mb-2">On this page</p>
          {NAV.map((item) => {
          ...
          <section id="coil-overview" className="rounded-lg border border-slate-200 bg-white p-5 mb-8 scroll-mt-28">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3">
              <div className="rounded-md border border-slate-200 bg-slate-50/50 px-3 py-3">
                ...
                <p className="text-lg font-semibold text-slate-900 tabular-nums">{receivedKg.toLocaleString()}</p>
              </div>
              <div className="rounded-md border border-slate-200 bg-slate-50/50 px-3 py-3">
                <p className="text-ui-xs font-medium text-slate-500">Kg used</p>
                <p className="text-lg font-semibold text-slate-900 tabular-nums">{kgUsed.toLocaleString()}</p>
              ...
              <div className="rounded-md border border-slate-200 bg-slate-50/50 px-3 py-3">
                <p className="text-ui-xs font-medium text-slate-500">Reserved</p>
                <p className="text-lg font-semibold text-slate-900 tabular-nums">{reservedKg.toLocaleString()}</p>
              </div>
              <div className="rounded-md border border-zarewa-teal/30 bg-zarewa-teal/5 px-3 py-3 col-span-2 sm:col-span-1">
                <p className="text-ui-xs font-medium text-slate-500">Free to use</p>
                <p className="text-lg font-semibold text-slate-900 tabular-nums">{freeKg.toLocaleString()}</p>
              </div>
            </div>
