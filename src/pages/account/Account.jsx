...
const Account = () => {
  ...
  const { domainLoading, domainReady } = useWorkspaceDomain(['finance', 'sales']);
  ...
  const [reversingRefundCreditAppId, setReversingRefundCreditAppId] = useState('');
...
<body>
  ...
  const openRequestPayment = useCallback((req) => {
    ...
    setSelectedPayment({
      ...
      maintenanceWorkOrderId: req.maintenanceWorkOrderId || '',
      maintenanceCostKind: req.maintenanceCostKind || '',
  ...
  const confirmProcessPaymentModal = async () => {
    ...
    if (ws?.canMutate) {
      ...
      try {
        const { ok, data } = await apiFetch(
          `/api/payment-requests/${encodeURIComponent(selectedPayment.id)}/pay`,
  ...
  const handleDeskViewRefund = useCallback(
    (refundId) => {
      const id = String(refundId || '').trim();
      const row = customerRefunds.find((r) => String(r.refundID || '').trim() === id);
      if (row) {
        setRefundViewTarget(row);
        return;
      }
      showToast(id ? `Refund ${id} is not on this desk.` : 'No refund selected.', { variant: 'info' });
    },
    [customerRefunds, showToast]
  );

  const handleDeskReverseRefundCredit = useCallback(
    async (applicationId) => {
      if (!ws?.hasPermission?.('finance.reverse')) {
        showToast('finance.reverse is required to undo a refund fund apply.', { variant: 'error' });
        return;
      }
      if (!ws?.canMutate) {
        showToast(
      ...
      if (!id || reversingRefundCreditAppId) return;
      if (
        !(await appConfirm({
          message:
            'Reverse this refund fund apply? The amount goes back onto the refund and is taken off the other quotation. This does not reverse till/bank cash.',
          variant: 'danger',
        }))
      ) {
        return;
      }
      setReversingRefundCreditAppId(id);
      try {
        const { ok, data } = await apiFetch('/api/ledger/reverse-refund-credit', {
          method: 'POST',
        ...
        setReversingRefundCreditAppId('');
      }
    },
    [showToast, ws, reversingRefundCreditAppId]
  );
  ...
  const saveTransfer = async (e) => {
    ...
    const transferOpts = withIdempotencyHeaders(
      {
        method: editingBatchId ? 'PATCH' : 'POST',
        body: JSON.stringify(body),
      },
      editingBatchId ? 'treasury_transfer_update' : 'treasury_transfer'
    );
    const { ok, data } = editingBatchId
      ? await apiFetch(`/api/treasury/transfer/${encodeURIComponent(editingBatchId)}`, transferOpts)
      : await apiFetch('/api/treasury/transfer', transferOpts);
    if (!ok || !data?.ok) {
  ...
  const deleteTreasuryTransfer = async (batchId) => {
    ...
    const { ok, data } = await apiFetch(
      `/api/treasury/transfer/${encodeURIComponent(id)}`,
      withIdempotencyHeaders({ method: 'DELETE' }, 'treasury_transfer_delete')
    );
  ...
  const financeSearchId = useId();
  const financePageSubtitle = (() => {
    if (activeTab === 'desk' && isCashierRole) {
      return 'Confirm what landed in till or bank, then pay approved items.';
    }
    if (activeTab === 'desk') {
  ...
  return (
    <AccountPageContext.Provider value={pageContextValue}>
      ...
      <WorkspaceDeskSyncBanner
        loading={domainLoading && !domainReady}
        label="finance register"
      />
      <FinancePilotHeader
        title={isCashierRole ? 'Cashier' : 'Finance'}
      ...
      <ModalFrame isOpen={showTransferModal} onClose={closeTransferModal} showCloseButton={false}>
      ...
      <ModalFrame isOpen={statementAccount != null} onClose={closeStatementModal} showCloseButton={false}>
      ...
      <ModalFrame isOpen={showStatementPrintModal} onClose={() => setShowStatementPrintModal(false)} showCloseButton={false}>
