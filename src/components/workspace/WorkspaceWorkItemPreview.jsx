...
function metaRow(label, value) {
  return (
    <div className="flex flex-col gap-0.5 border-b border-slate-100 py-2.5 last:border-0 sm:flex-row sm:items-baseline sm:gap-4">
      <span className="w-28 shrink-0 text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</span>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white">
      <div className="flex shrink-0 flex-wrap items-center gap-1.5 border-b border-slate-200 bg-gradient-to-r from-teal-50/50 to-white px-3 py-2">
        <span className="rounded-md bg-teal-100/90 px-2 py-0.5 text-xs font-semibold text-teal-950 ring-1 ring-teal-200/60">
          {tabLabel}
        </span>
        <span className="rounded-md bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-700">Work item</span>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
        ...
        {item.summary ? (
          <div className="mt-5 border-t border-slate-100 pt-4">
            <p className="text-ui-xs font-semibold uppercase tracking-wide text-slate-500">Summary</p>
        ...
        {tid ? (
          <div className="mt-8">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-zarewa-teal px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[#0f3d3a]"
            >
              Open linked memo
            </button>
          </div>
