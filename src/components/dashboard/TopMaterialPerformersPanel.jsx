      ...
      <div className="flex flex-col gap-4 mb-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-start gap-3 min-w-0">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-zarewa-teal">
          ...
          <button
            type="button"
            onClick={onOpenSales}
            className="text-ui-xs font-semibold text-zarewa-teal uppercase tracking-wide hover:underline shrink-0 self-start sm:self-auto"
          >
            Sales detail
          </button>
        </div>
        <div className="inline-flex rounded-lg border border-slate-200 p-0.5 bg-slate-50 w-full sm:w-auto">
          <span className="px-2.5 sm:px-3 py-1.5 rounded-md text-ui-xs font-semibold uppercase tracking-wide bg-white text-zarewa-teal shadow-sm border border-slate-200/80">
            This month (MTD)
          </span>
        </div>
      </div>

      <div className="hidden sm:grid sm:grid-cols-[2.5rem_minmax(0,4.5rem)_minmax(0,5rem)_minmax(0,1fr)_minmax(0,10.5rem)_minmax(0,7rem)] gap-x-3 gap-y-1 px-3 py-2 border-b border-slate-200 text-ui-xs font-semibold uppercase tracking-wider text-slate-400">
          ...
          {rows.map((row) => (
            <li key={`${row.rank}-${row.colour}-${row.gaugeRaw}-${row.materialType}`}>
              <button
                type="button"
                onClick={onOpenSales}
                className="w-full text-left py-3 px-2 sm:px-3 rounded-lg hover:bg-slate-50/80 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zarewa-teal/15"
              >
                <div className="sm:hidden space-y-1">
                  <div className="flex items-center justify-between gap-2">
                    <span className="flex h-7 w-7 items-center justify-center rounded-md bg-zarewa-teal text-ui-xs font-bold text-white tabular-nums shrink-0">
                      {row.rank}
                    </span>
                  ...
                  <p className="text-xs text-slate-600 pl-9">{row.materialType}</p>
                  <div className="flex flex-wrap items-baseline justify-between gap-x-2 gap-y-1 pl-9 text-xs tabular-nums">
                    <span className="text-slate-500">
                      {row.metresProduced.toLocaleString()} m · ~{row.weightKg.toLocaleString()} kg
                    </span>
                    <span className="font-semibold text-zarewa-teal">{formatNgn(row.revenueNgn)}</span>
                  </div>
                </div>
                <div className="hidden sm:grid sm:grid-cols-[2.5rem_minmax(0,4.5rem)_minmax(0,5rem)_minmax(0,1fr)_minmax(0,10.5rem)_minmax(0,7rem)] gap-x-3 items-center">
                  <span className="flex h-8 w-8 mx-auto items-center justify-center rounded-md bg-slate-100 text-xs font-bold text-zarewa-teal tabular-nums">
                    {row.rank}
                  </span>
                  ...
                  <span className="text-sm font-semibold text-zarewa-teal tabular-nums text-right">
