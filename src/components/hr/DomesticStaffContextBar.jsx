  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-amber-100 bg-gradient-to-r from-amber-50/80 to-white px-4 py-3 shadow-sm">
      <div className="min-w-0">
        {employer ? (
          <p className="text-xs font-bold uppercase tracking-wide text-amber-800">{employer}</p>
        ) : null}
      ...
      {profile.employeeNo ? (
        <span className="shrink-0 rounded-full border border-amber-200 bg-white px-2.5 py-0.5 text-xs font-semibold text-amber-900">
          {profile.employeeNo}
        </span>
      ) : null}
    </div>
