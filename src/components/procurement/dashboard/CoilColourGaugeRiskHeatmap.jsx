...
export default function CoilColourGaugeRiskHeatmap({ rows = [] }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-3">
      <h4 className="text-ui-xs font-bold uppercase tracking-wide text-slate-500">Coil risk (colour × gauge)</h4>
      <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {rows.length === 0 ? <p className="text-xs text-slate-400">No coil risk rows</p> : null}
        {rows.map((r) => (
          <div key={r.key} className={`rounded-lg border px-2.5 py-2 text-ui-xs ${riskClass(r.daysCover)}`}>
