...
export default function ProcurementCategoryDonut({ rows = [] }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-3">
      <h4 className="text-ui-xs font-bold uppercase tracking-wide text-slate-500">Purchase by category</h4>
        ...
        {rows.map((r) => {
          ...
          return (
            <div key={r.key} className="rounded-lg border border-slate-100 p-2">
              ...
              <p className="mt-0.5 text-xs font-bold text-zarewa-teal tabular-nums">{formatNgn(r.value)}</p>
            </div>
          );
        })}
      </div>
