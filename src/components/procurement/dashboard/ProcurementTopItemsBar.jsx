...
export default function ProcurementTopItemsBar({ rows = [] }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-3">
      <h4 className="text-ui-xs font-bold uppercase tracking-wide text-slate-500">Top purchased items</h4>
        ...
        {rows.map((r) => (
          <div key={r.itemKey} className="space-y-1">
            <div className="flex items-center justify-between text-ui-xs">
              ...
              <span className="font-bold tabular-nums text-zarewa-teal">{formatNgn(r.spendNgn)}</span>
            </div>
