...
function KpiCard({ label, value, hint, tone = 'default', action, info }) {
  ...
  const valueTones = {
    default: 'text-zarewa-teal',
    amber: 'text-amber-950',
    violet: 'text-violet-950',
    teal: 'text-zarewa-teal',
  };

  return (
    <div className={`rounded-2xl border p-4 shadow-sm ${tones[tone] || tones.default}`}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <p className={`text-ui-xs font-black uppercase tracking-widest ${labelTones[tone] || labelTones.default}`}>
          {label}
        </p>
        {info ? (
          <details className="relative shrink-0">
            ...
            <div className="absolute right-0 top-full z-40 mt-1.5 w-[min(calc(100vw-2rem),17.5rem)] rounded-xl border border-slate-200 bg-white p-3 text-ui-xs leading-snug text-slate-700 shadow-lg">
              {info}
            </div>
          </details>
        ) : null}
      ...
      {hint ? <p className="text-ui-xs text-slate-500 mt-2 leading-snug">{hint}</p> : null}
      {action}
    </div>
  );
}
        ...
        action={
          advanceBalNgn > 0 && onRefundAdvance ? (
            <button
              ...
              className="mt-2 text-ui-xs font-bold uppercase text-amber-900 hover:underline"
            >
              Refund advance
            </button>
          ) : null
      ...
      <KpiCard
        ...
        action={
          <div className="mt-2 h-1.5 rounded-full bg-slate-100 overflow-hidden">
            <div
              className="h-full rounded-full bg-gradient-to-r from-zarewa-teal to-teal-400 transition-all"
