        ...
        <div className="border-t border-amber-200/70 px-3 pb-3 sm:px-4 sm:pb-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="rounded-lg border border-white/80 bg-white/90 p-3 shadow-sm">
              ...
              <ul className="mt-2 space-y-1.5 text-ui-xs font-semibold text-slate-700">
                <li className="flex justify-between gap-2">
                  <span>WIP rows (non-zero)</span>
                  <span className="tabular-nums text-zarewa-teal">{ic.wipProductsNonZero ?? 0}</span>
                </li>
                <li className="flex justify-between gap-2">
                  <span>FG completion adjustments (30d)</span>
                  <span className="tabular-nums text-zarewa-teal">{ic.completionAdjustmentsLast30d ?? 0}</span>
                </li>
                <li className="flex justify-between gap-2">
                  <span>Deliveries in progress</span>
                  <span className="tabular-nums text-zarewa-teal">{ic.deliveriesInProgress?.count ?? 0}</span>
                </li>
              </ul>
            ...
            <div className="rounded-lg border border-white/80 bg-white/90 p-3 shadow-sm">
              ...
              <ul className="mt-2 space-y-1.5 text-ui-xs font-semibold text-slate-700">
                <li className="flex justify-between gap-2">
                  ...
                  <span className="tabular-nums text-zarewa-teal">{cm.partialPurchaseOrderCount ?? 0}</span>
                </li>
                <li className="flex justify-between gap-2">
                  <span>Open in-transit loads</span>
                  <span className="tabular-nums text-zarewa-teal">{cm.openInTransitLoadCount ?? 0}</span>
                </li>
              </ul>
              <button
                type="button"
