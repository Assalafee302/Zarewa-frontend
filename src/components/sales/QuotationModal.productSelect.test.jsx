...
describe('QuotationModal product select (live #185 path)', () => {
  ...
  it('selecting Roofing Sheet after material header does not exceed update depth', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByPlaceholderText(/name, phone, or staff id/i)).toBeTruthy();
    });

    await user.type(screen.getByPlaceholderText(/name, phone, or staff id/i), 'Acme');
    await user.click(await screen.findByRole('button', { name: /acme/i }));
    await user.type(screen.getByPlaceholderText(/alh musa house/i), 'Site A');
