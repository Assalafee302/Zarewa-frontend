...
import { StaffPurchaseCreditQuotationPanel } from './StaffPurchaseCreditQuotationPanel';
...
function normalizeLoadedLines(raw) {
  ...
  const mapRow = (r) => {
    const x = {
      ...
      assigneeCustomerID: r.assigneeCustomerID != null ? String(r.assigneeCustomerID) : '',
      assigneeAssociatedStaffID:
        r.assigneeAssociatedStaffID != null
          ? String(r.assigneeAssociatedStaffID)
          : r.assigneeCustomerID != null
            ? String(r.assigneeCustomerID)
            : '',
      assigneeName: r.assigneeName != null ? String(r.assigneeName) : '',
      assigneeRole: r.assigneeRole != null ? String(r.assigneeRole) : '',
    };
      ...
      <div className="bg-slate-50/80 rounded-xl p-3 sm:p-4 border border-slate-200/90">
        {readOnly ? (
              ...
              >
                <span className="font-semibold text-zarewa-teal">
                  ...
                  {title === 'Products' && isQuotationTrimProductLine(row.name) && row.girthMm
                    ? ` · ${row.girthMm} mm`
                    : null}
                  {title === 'Services' && row.assigneeName
                    ? ` · ${row.assigneeName}${row.assigneeRole ? ` (${row.assigneeRole})` : ''}`
                    : null}
                </span>
          ...
          <>
              ...
              return (
                ...
                >
                  ...
                  <input
                    ...
                    placeholder={isStoneFlatsheetRow || quoteItemUnitIsArea(matchedOption?.unit) ? 'm²' : '0'}
                  />
            ...
            {title === 'Services' && showServiceAssignees ? (
              <div className="mt-2 space-y-2">
                {rows.map((row) => {
                  const expectedRole = serviceAssigneeRole(row.name);
                  if (!expectedRole) return null;
                  const roleOptions = serviceAssigneeOptions.filter((opt) =>
                    expectedRole === 'driver'
                      ? opt.staffType.includes('driver')
                      : opt.staffType.includes('install')
                  );
                  if (roleOptions.length === 0) {
                    return (
                      <div
                        key={`${row.id}:assignee-empty`}
                        className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-ui-xs text-amber-900"
                      >
                        No active {expectedRole === 'driver' ? 'driver' : 'installer'} profiles yet for{' '}
                        <span className="font-semibold">{row.name || 'this service'}</span>. Register partners under{' '}
                        <a href="/manager?tab=branch#manager-associated-staff" className="font-bold text-zarewa-teal underline">
                          Branch Manager → Installers &amp; Drivers
                        </a>
                        .
                      </div>
                    );
                  ...
                  return (
                    <div key={`${row.id}:assignee`} className="flex items-center gap-2 pl-1">
                      <div className="w-24 shrink-0 text-ui-xs font-semibold text-slate-500 uppercase">
                        {expectedRole === 'driver' ? 'Transporter' : 'Installer'}
                      </div>
                      <select
                        value={row.assigneeAssociatedStaffID || row.assigneeCustomerID || ''}
                        onChange={(e) => {
                          const customerId = e.target.value;
                          const picked = roleOptions.find((x) => x.customerID === customerId);
                          updateRow(row.id, {
                            assigneeAssociatedStaffID: customerId,
                            assigneeCustomerID: customerId,
                            assigneeName: picked?.name || '',
                            assigneeRole: expectedRole,
                          });
                        }}
                        className="min-w-0 flex-1 bg-white border border-slate-200 rounded-lg py-1.5 px-2 text-xs font-semibold text-zarewa-teal outline-none focus:ring-2 focus:ring-zarewa-teal/15"
                      >
                        <option value="">Select {expectedRole}</option>
                        {roleOptions.map((opt) => (
...
const QuotationModal = ({
  ...
  /** Compose wizard: Customer → Lines → Pay. View mode shows every section. */
  const [composeStep, setComposeStep] = useState('customer');
  ...
  const associatedStaffPolicyEnabled = Boolean(ws?.snapshot?.associatedStaffPolicy?.enabled);
  const currentUserHandledByLabel = useMemo(() => {
  ...
  const serviceAssigneeOptions = useMemo(() => {
    const rows = Array.isArray(ws?.snapshot?.associatedStaff) ? ws.snapshot.associatedStaff : [];
    return rows
      .filter((c) => String(c?.status || 'Active').trim().toLowerCase() === 'active')
      .map((c) => ({
        customerID: c.id,
        name: c.name,
        staffType: String(c?.staffType || c?.staff_type || '').trim().toLowerCase(),
      }))
  ...
  const customerReady = Boolean(selectedCustomer?.customerID && String(projectName || '').trim());
  const linesReady = Boolean(
    String(materialTypeId ?? '').trim() &&
      String(materialGauge ?? '').trim() &&
      String(materialColor ?? '').trim() &&
      String(materialDesign ?? '').trim()
  );
  const showComposeCustomer = readOnly || composeStep === 'customer';
  const showComposeLines = readOnly || composeStep === 'lines';
  const showComposePay = readOnly || composeStep === 'pay';

  const goComposeBack = () => {
    if (composeStep === 'pay') setComposeStep('lines');
    else if (composeStep === 'lines') setComposeStep('customer');
  };

  const goComposeNext = () => {
    if (composeStep === 'customer') {
      ...
      setComposeStep('lines');
      return;
    }
    if (composeStep === 'lines') {
      if (!linesReady) {
        showToast(
          materialHeaderIncompleteMessage(materialTypeId, materialGauge, materialColor, materialDesign) ||
            'Complete material type, gauge, colour, and profile.',
          { variant: 'error' }
        );
        return;
      }
  ...
  const preparedByLabel = String(handledByStaff || quotedByStaff || '').trim();
  ...
  const buildLinesPayload = () => {
    ...
    return {
      ...
      services: serviceRows.map((row) => ({
        ...
        design: materialDesign,
        profile: materialDesign,
        assigneeAssociatedStaffID: String(row.assigneeAssociatedStaffID || '').trim(),
        assigneeCustomerID: String(row.assigneeCustomerID || '').trim(),
        assigneeName: String(row.assigneeName || '').trim(),
        assigneeRole: String(row.assigneeRole || serviceAssigneeRole(row.name) || '').trim(),
      })),
    };
  };
    ...
    <ModalFrame isOpen={isOpen} onClose={handleClose} modal={!showPrint} showCloseButton={false}>
      ...
      >
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-ui-xs font-medium text-slate-500 mt-1">
                Handled by: <span className="font-semibold text-zarewa-teal">{preparedByLabel}</span>
                {handleOnBehalfOf && currentUserHandledByLabel ? (
        ...
        <div className="flex-1 overflow-y-auto p-5 custom-scrollbar bg-white">
          ...
          <div className="rounded-xl border border-slate-200/90 p-4 mb-5 bg-white">
            ...
            {selectedCustomerId ? (
              ...
              <p className="mt-1.5 text-ui-xs text-slate-500 leading-snug">
                Commercial account for this job (the buyer, or the dealer who brought the client).
              </p>
            )}
            {!readOnly && customerQuery.trim().length >= 2 && filteredCustomers.length === 0 ? (
              ...
              <div>
            <label className="text-ui-xs font-semibold text-slate-500 uppercase tracking-widest mb-2 block">
              Client name / site <span className="text-rose-600 normal-case font-bold">(required)</span>
            </label>
            <input
              type="text"
              value={projectName}
              ...
              readOnly={readOnly}
              required={!readOnly}
              placeholder="e.g. Alh Musa house"
            ...
            />
            <p className="mt-1.5 text-ui-xs text-slate-500 leading-snug">
              End client for this job (e.g. Alh Musa house). Printed on quotation and invoice.
            </p>
              </div>
            </div>
            <div className="mt-4 max-w-lg">
              ...
              {!handleOnBehalfOf ? (
                <div className="flex flex-wrap items-center gap-2">
                  <div className="min-w-0 flex-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs font-semibold text-zarewa-teal">
                    {handledByStaff || currentUserHandledByLabel || '—'}
                  </div>
                  {!readOnly ? (
                    <button
                      type="button"
                      ...
                      title="Enter this quotation for another staff member"
                    >
                      <Users size={14} />
                      On behalf of…
                    </button>
          ...
          <OrderLinesSection
            ...
            readOnly={readOnly}
            resolveUnitPrice={resolveUnitPrice}
            serviceAssigneeOptions={serviceAssigneeOptions}
            showServiceAssignees={associatedStaffPolicyEnabled}
          />
          </>
          ) : null}
        </div>
        ...
        <ModalDeskFooter totalValue={formatNgn(grandTotalNgn)}>
          {!readOnly && composeStep !== 'customer' ? (
            <DeskFooterButton type="button" onClick={goComposeBack}>
              Back
            </DeskFooterButton>
          ) : null}
          {!readOnly && composeStep !== 'pay' ? (
            <DeskFooterButton type="button" variant="primary" onClick={goComposeNext}>
              Continue
            </DeskFooterButton>
          ) : null}
          <DeskFooterButton
