...
/**
 * @param {{ categoryFilter?: string | null; compact?: boolean }} props
 */
export function ProfileActionGrid({ categoryFilter = null, compact = false }) {
  ...
  const actions = useMemo(
    ...
    [ws?.permissions, ws?.session?.user?.roleKey, ws?.canAccessModule]
  );
  ...
  if (filtered.length === 0) {
    return (
      <p className="text-sm text-slate-500">
        No extra actions for your role. You can still update your account and password below.
      </p>
    );
  }
  ...
  return (
    ...
    >
      <div className="min-w-0">
        <p className={`font-bold text-slate-900 ${compact ? 'text-sm' : 'text-[15px]'}`}>{action.label}</p>
