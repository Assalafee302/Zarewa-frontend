...
const KPI_CARD =
  'rounded-xl border border-slate-200/90 bg-white shadow-sm p-4 text-left transition-colors hover:border-[#134e4a]/20';
...
export function ExecMdKpiRow({
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4 mb-6">
      ...
      >
        <p className="text-[10px] font-semibold uppercase tracking-widest text-violet-800 flex items-center gap-1.5">
      ...
      <div className={`${KPI_CARD} border-teal-200/90 bg-teal-50/30`}>
        <p className="text-[10px] font-semibold uppercase tracking-widest text-teal-700 flex items-center gap-1.5">
          <Award size={14} strokeWidth={2} /> Champion customer
        </p>
        <p className="mt-2 text-sm font-bold text-[#134e4a] leading-tight line-clamp-2">
      ...
      <div className={KPI_CARD}>
        ...
        <p className="mt-2 text-2xl font-black text-[#134e4a] tabular-nums">
      ...
      <button type="button" onClick={onOpenDecide} className={KPI_CARD}>
        ...
        <p className="mt-2 text-2xl font-black text-[#134e4a] tabular-nums">{pendingActions ?? 0}</p>
