...
function PulseCard({ label, detail, status, estimated }) {
  ...
  return (
    <div className={`rounded-lg border px-3 py-2.5 min-w-0 ${tone}`}>
      <p className="text-[9px] font-semibold uppercase tracking-wide text-slate-500 flex items-center gap-1">
        {label}
        {estimated ? (
          <span className="rounded px-1 py-0.5 text-[7px] bg-white/70 border border-slate-200/80">Est.</span>
        ) : null}
      </p>
      <p className="mt-1 text-[11px] font-bold leading-snug text-slate-900 truncate">{detail}</p>
    </div>
  );
}
  ...
  return (
    <section className="rounded-xl border border-slate-200/90 bg-white shadow-sm overflow-hidden mb-6">
      ...
      <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-2">
        <Activity size={14} className="text-[#134e4a] shrink-0" strokeWidth={2} />
        <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-500">Company pulses</p>
      </div>
