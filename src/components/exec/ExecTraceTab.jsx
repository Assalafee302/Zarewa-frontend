    ...
    <article className="rounded-xl border border-slate-200/90 bg-white shadow-sm overflow-hidden">
      ...
      <div className="px-4 sm:px-5 py-4 border-b border-slate-100 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-ui-xs font-black uppercase tracking-widest text-slate-400">
            {sample.domainLabel || sample.domain}
          </p>
          <h3 className="text-base font-bold text-zarewa-teal truncate">{sample.title}</h3>
        ...
        <span
          className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-ui-xs font-black uppercase ring-1 ${OUTCOME_CHIP[outcome] || OUTCOME_CHIP.partial}`}
        >
      ...
      <ol className="px-4 sm:px-5 py-4 space-y-3">
        {(sample.timeline || []).map((t, i) => (
          <li key={i} className="flex gap-3 text-xs">
            <span className="shrink-0 w-28 font-black uppercase text-ui-xs text-slate-500 leading-snug">
              {t.step}
            </span>
            <div className="min-w-0 flex-1">
              <p className="font-semibold text-slate-800">{t.detail}</p>
              {t.atIso ? <p className="text-ui-xs text-slate-500 mt-0.5">{t.atIso.slice(0, 16)}</p> : null}
              {t.docRef ? (
                <button
                  type="button"
                  onClick={() => onOpenRef?.(sample.domain, t.docRef)}
                  className="mt-1 text-ui-xs font-bold text-zarewa-teal hover:underline"
                >
                  Open {t.docRef}
                </button>
              ) : null}
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3">
        <div>
          <p className="text-sm font-bold text-zarewa-teal">MD Trace</p>
          <p className="text-xs text-slate-600 mt-0.5">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            onClick={onReload}
            disabled={busy}
            className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-ui-xs font-bold uppercase text-slate-700 hover:bg-slate-50 disabled:opacity-50"
          >
          ...
          <button
            ...
            onClick={onShuffle}
            disabled={busy}
            className="inline-flex items-center gap-1.5 rounded-lg bg-zarewa-teal px-3 py-1.5 text-ui-xs font-bold uppercase text-white hover:brightness-105 disabled:opacity-50"
          >
            <Shuffle size={12} />
            Shuffle sample
          </button>
      ...
      {(data?.notes || []).length ? (
        <ul className="text-ui-xs text-slate-500 space-y-1 list-disc pl-4">
          {data.notes.map((n, i) => (
            <li key={i}>{n}</li>
          ))}
        </ul>
