/**
 * Primary navigation chrome. Keep permission checks here in sync with App.jsx route guards.
 */
...
import { workspaceDomainsForPath } from '../../lib/workspaceDomainPrefetch';
...
const Sidebar = ({
  ...
  const warmNavTarget = (path) => {
    const domains = workspaceDomainsForPath(path);
    if (!domains.length || ws?.status !== 'ok') return;
    void ws.prefetchWorkspaceDomains?.({ only: domains, priorityDomain: domains[0] });
  };
  ...
  const linkClass = (active) =>
    `w-full flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all duration-300 group no-underline ${
      ...
      active
        ? 'bg-white/10 text-[var(--z-nav-active)] shadow-inner'
          ...
          return (
            ...
            >
              <Link
                ...
                onMouseEnter={() => warmNavTarget(item.path)}
                onFocus={() => warmNavTarget(item.path)}
