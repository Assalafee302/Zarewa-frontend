    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {data ? (
        <button
          type="button"
          onClick={exportCsv}
          className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-ui-xs font-bold uppercase text-slate-600"
        >
          Export CSV
        </button>
      ) : null}
    ...
    <section className="space-y-4">
      {!deskLayout ? (
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h3 className="text-lg font-bold text-zarewa-teal">Branch contribution P&L</h3>
            <p className="mt-1 text-ui-xs text-slate-500 max-w-2xl">
      ...
      {data ? (
        <>
          ...
          <p className="text-ui-xs text-slate-500">{data.disclaimer}</p>
        </>
      ) : null}
    </section>
  );
