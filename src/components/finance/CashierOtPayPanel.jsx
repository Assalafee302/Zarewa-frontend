 ...
 */
export function CashierOtPayPanel({ onPaid }) {
  ...
  return (
    <>
      <FinanceDeskColoredQueuePanel
        ...
        theme="teal"
        title="Overtime pay — approved, awaiting payout"
        icon={<Banknote size={16} strokeWidth={2} />}
        count={rows.length}
        description="Branch manager locks the payable at approval. Post bank/cash payout here the same way as refunds and expenses."
        testId="cashier-ot-pay-panel"
        action={
          <button
            type="button"
            onClick={() => void load()}
            className="inline-flex items-center gap-1 text-ui-xs font-bold uppercase text-teal-900 hover:underline"
          >
