      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p
            ...
            className="text-ui-xs font-black uppercase tracking-[0.2em] text-zarewa-teal/70"
          >
            Your workspace
          </p>
          <p className="mt-1 text-sm font-bold text-zarewa-teal">{deptLabel}</p>
          <p className="mt-1 max-w-2xl text-xs leading-relaxed text-slate-600">{entry.primary}</p>
        </div>
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-zarewa-teal/10 text-zarewa-teal">
        ...
        {links.map((link, idx) => (
          <li key={`${link.to}-${link.label}-${idx}`}>
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-full border border-teal-200/80 bg-white px-3 py-1.5 text-xs font-bold uppercase tracking-wide text-zarewa-teal shadow-sm transition hover:border-teal-300 hover:bg-teal-50/80"
            >
              {link.label}
