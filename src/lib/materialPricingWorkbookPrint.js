...
/**
 * @typedef {{ gaugeMm: string; conversionUsedKgPerM: string; costPerKgNgn: string; suggestedNgn: number | null; minimumNgn: number }} MaterialPricingPrintRow
 */
...
export function printMaterialPricingWorkbookSummary({ materialLabel, branchName, rows, formatNgn }) {
        ...
        .map((r) => {
          ...
          const floor =
            r.minimumNgn != null && Number.isFinite(r.minimumNgn) && r.minimumNgn > 0
              ...
              : '—';
          const used = String(r.conversionUsedKgPerM || '').trim() || '—';
  ...
  <td class="num">${escapeHtml(floor)}</td>
        ...
        .join('')
    : `<tr><td colspan="5" class="muted">No gauges with a suggested or minimum price to list.</td></tr>`;

  w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"/>
<title>${escapeHtml(materialLabel)} — material pricing</title>
<style>
  body{font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;padding:24px 28px;color:#0f172a;max-width:720px;margin:0 auto;}
  ...
  th{background:#f1f5f9;font-weight:700;font-size:10px;text-transform:uppercase;letter-spacing:0.04em;color:#475569;}
  ...
  <p style="font-size:11px;color:#64748b;margin:0 0 8px;">Listed gauges have a suggested and/or minimum floor price.</p>
  <table>
    <thead>
      <tr>
        <th>Gauge</th>
        <th class="num">Used kg/m</th>
        <th class="num">₦/kg</th>
  ...
  <p class="foot">Zarewa — internal pricing reference. Quotations below published minima require MD approval where applicable.</p>
</body></html>`);
  w.document.close();
  w.focus();
  const triggerPrint = () => {
