...
export const WORKSPACE_DEPARTMENT_LABELS = {
  ...
  gmhr: 'General Manager, HR',
  hr_portal_only: 'Employee self-service',
...
export const WORKSPACE_GUIDE_ENTRIES = [
  ...
  },
  {
    ...
    primary:
      'Sourcing and supplier performance — purchase orders, invoices on file, coordination with store GRN. In this deployment procurement authority sits with the managing director role.',
    bullets: [
      'Suppliers: directory and transport agents (Purchases / Transportation tabs).',
      'Purchase orders: multi-line POs, totals, Pending → Approved / Rejected; assign transport (loading confirmed), then post to in transit (optional treasury-linked haulage) before GRN.',
      'Quantities finalized at store receipt (GRN).',
      'Supplier payables and payments: Procurement → Supplier payables tab (same module as POs).',
    ],
  ...
  },
  {
    ...
    bullets: [
      ...
      'Supplier AP: procurement posts supplier payments; Finance Desk / Treasury posts expense, refund, and haulage payouts.',
      'Payouts & expenses tab: posted treasury debits (audit) plus request pipeline; pay new items from Finance desk or Treasury.',
      'Control: reconciliation lines, audit checklist, reporting exports (stubs).',
    ],
    links: [{ to: '/accounts', label: 'Finance' }],
  },
  {
    id: 'reports',
