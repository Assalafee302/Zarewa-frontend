...
/** Cash actually received for a sales receipt (reconciled bank amount when finance confirmed). */
export function receiptCashReceivedNgn(r) {
  return receiptEffectiveCashNgn(r);
}
...
export function salesReceiptMirrorClearanceFields(mirror) {
  ...
  return {
    ...
    bankConfirmedBy: mirror.bankConfirmedBy ?? null,
    ...
    financeDeliveryClearedBy: mirror.financeDeliveryClearedBy ?? null,
...
export function mergeReceiptRowsForSales(importedReceipts, quotations, _ledgerEpoch = 0) {
  ...
  const importedRows = (importedReceipts || [])
    ...
    .map((r) => {
      ...
      const hint = quotePaymentHint(q);
      const extra =
        companionOverpay.get(String(r.id || '')) ||
        (r.ledgerEntryId ? companionOverpay.get(String(r.ledgerEntryId)) : 0) ||
        0;
      ...
      return {
        ...
        financeReconciliationSavedAtISO: r.financeReconciliationSavedAtISO ?? null,
        _payBadge: hint.badge,
...
export function quotationReceiptPrintHistory(quotationId, importedReceipts = []) {
  ...
  const ledger = ledgerEntries
    ...
    .map((e) => {
      ...
      const mirror = mirrorById.get(String(e.id || ''));
      const receiptRow = {
        amountNgn: alloc,
        financeReconciliationSavedAtISO: mirror?.financeReconciliationSavedAtISO ?? null,
        bankReceivedAmountNgn: mirror?.bankReceivedAmountNgn ?? null,
      ...
      return {
        ...
        amountNgn: receiptEffectiveCashNgn(receiptRow, { companionOverpayNgn: extra }),
        source: 'Ledger',
