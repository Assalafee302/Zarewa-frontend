...
export function resolveManagerTargets(targets, scope = {}) {
  ...
  const pick = (...layers) => {
    ...
    for (const layer of layers) {
      ...
      if (naira == null && Number(layer.nairaTargetPerMonth) > 0) {
        naira = Number(layer.nairaTargetPerMonth);
      }
      if (metre == null && Number(layer.meterTargetPerMonth) > 0) {
        metre = Number(layer.meterTargetPerMonth);
      }
    }
  ...
  let source = 'company';
  if (byBq && (Number(byBq.nairaTargetPerMonth) > 0 || Number(byBq.meterTargetPerMonth) > 0)) {
    source = 'branch_quarter';
  } else if (byB && (Number(byB.nairaTargetPerMonth) > 0 || Number(byB.meterTargetPerMonth) > 0)) {
    source = 'branch';
  } else if (byQ && (Number(byQ.nairaTargetPerMonth) > 0 || Number(byQ.meterTargetPerMonth) > 0)) {
    source = 'quarter';
  }
