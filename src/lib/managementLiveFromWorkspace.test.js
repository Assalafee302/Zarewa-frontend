...
describe('topCustomersByNetPaymentsAndMeters', () => {
  it('ranks by cutting-list metres first, then net payments minus refunds', () => {
    ...
    const rows = topCustomersByNetPaymentsAndMeters(
      ...
      [
        ...
        { customerID: 'C2', dateISO: '2026-05-13', totalMeters: 2000 },
      ],
      quoteById,
      '2026-05-01',
      5
    );
    expect(rows[0].customer_id).toBe('C2');
    expect(rows[0].cuttingListMeters).toBe(2000);
    expect(rows[0].netCollectedNgn).toBe(300_000);
    expect(rows[1].customer_id).toBe('C1');
    expect(rows[1].netCollectedNgn).toBe(450_000);
    expect(rows[1].cuttingListMeters).toBe(1200);
  });
});
